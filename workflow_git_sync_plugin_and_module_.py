...
def process_repository(repo_config, target_dir):
    """
    Clones/updates a repository in a specific target directory.
    """
    ...
    if os.path.exists(os.path.join(target_dir, ".git")):
        print(f"--- Directory exists as a Git repo. Attempting to pull updates in '{target_dir}' ---")
        try:
            run_command(f"git pull origin {branch}", working_dir=target_dir)
        except subprocess.CalledProcessError:
            print(f"Warning: Could not pull updates for '{target_dir}'. There might be local changes.", file=sys.stderr)

    else:
        # --- 2. If not, perform the initial clone or sparse checkout ---
        print(f"--- Setting up new repository in '{target_dir}' ---")
        
        # Safety Check: Do not delete the CWD. This case should be handled by the pull logic above.
        if os.path.exists(target_dir) and os.path.samefile(target_dir, cwd):
             print(f"Error: Target directory is the current directory but is not a Git repo.", file=sys.stderr)
             print("Please initialize it (`git init`) or run the script from the parent directory.", file=sys.stderr)
             return
        ...
        if is_sparse:
            # ... (sparse checkout logic remains the same as your previous version)
            ...
            # Full clone
            run_command(f'git clone --branch {branch} {git_url} .', working_dir=target_dir)
...
def main():
    ...
    # Scenario B: Check if we are in the parent container directory
        ...
        # Invalid Location
        print("Validation Error: Script not run from a recognized directory.", file=sys.stderr)
        expected_parent = f"'{must_be_in[-1]}'" if must_be_in else "a configured container"
        print(f"Please run this script from within a specific plugin directory or from the main container directory ({expected_parent}).", file=sys.stderr)
        sys.exit(1)
